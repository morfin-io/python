...
def apply_from_yaml_single_item(
    ...
    if not resource_exists(k8s_api, resource_name, kind, namespace, **kwargs):
...
def resource_exists(k8s_api, name, kind, namespace, **kwargs):
    # Expect the user to create namespaced objects more often
    try:
        if hasattr(k8s_api, "read_namespaced_{0}".format(kind)):
            resp = getattr(k8s_api, "read_namespaced_{0}".format(kind))(
                name=name, namespace=namespace, **kwargs)
        else:
            resp = getattr(k8s_api, "read_{0}".format(kind))(
                name=name, **kwargs)
        return True
    except client.rest.ApiException as api_exception:
